...
// Only allow WhatsApp deep links. The `wa` param is attacker-controllable, so an
// unvalidated href turns this page into an open redirect (?wa=https://evil.com).
// Anything that isn't an https wa.me / api.whatsapp.com URL falls back to the
// official number.
function safeWhatsAppUrl(raw: string | null, fallbackPhone: string): string {
  const fallback = `https://wa.me/${fallbackPhone}`;
...
function ConfirmacionContent() {
  ...
  const waHref = safeWhatsAppUrl(searchParams.get('wa'), CONTACT.whatsapp);
    ...
    <ConfettiBackground className="bg-beige min-h-[70vh]">
      <div className="max-w-2xl mx-auto px-4 py-16 md:py-24 text-center space-y-6">
        ...
        <div className="bg-orange/10 border border-orange/30 rounded-xl p-4 max-w-md mx-auto">
          <p className="font-heading font-bold text-sm text-orange-text">
        ...
        {showBankInfo && (
          <div className="bg-white rounded-2xl border border-gray-100 p-5 text-left max-w-sm mx-auto space-y-2 shadow-sm">
            ...
            <button
              onClick={copyBank}
              className="w-full mt-2 bg-teal/10 text-teal-text font-heading font-bold text-sm py-2.5 rounded-xl hover:bg-teal/20 transition-colors"
            >
        ...
        <div className="flex flex-col gap-4 justify-center pt-2 max-w-sm mx-auto">
          <a href={waHref} target="_blank" rel="noopener noreferrer">
            <Button size="lg" className="bg-[#128C7E] hover:bg-[#0e7566] text-white w-full shadow-lg animate-pulse">
