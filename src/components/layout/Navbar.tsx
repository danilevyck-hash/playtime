      ...
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between relative">
        ...
        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} aria-current={isActive ? 'page' : undefined} className={`font-heading font-bold transition-colors ${isActive ? 'text-teal-text' : 'text-gray-600 hover:text-teal-text'}`}>
                {link.label}
              </Link>
            );
          })}
          <Link href="/carrito" aria-label={`Carrito${itemCount > 0 ? ` (${itemCount})` : ''}`} className={`relative flex items-center gap-1.5 transition-colors ${itemCount > 0 ? 'bg-teal/10 rounded-full px-3 py-1.5' : ''}`}>
        ...
        <div className="flex md:hidden items-center gap-3">
          <Link href="/carrito" aria-label={`Carrito${itemCount > 0 ? ` (${itemCount})` : ''}`} className={`relative flex items-center gap-1.5 min-h-[44px] transition-colors ${itemCount > 0 ? 'bg-teal/10 rounded-full px-2.5' : 'px-1'}`}>
          ...
          <button onClick={() => setMenuOpen(true)} className="p-3 -mr-3 min-h-[44px] min-w-[44px]" aria-label="Abrir men\u00fa" aria-expanded={menuOpen} aria-haspopup="dialog">
